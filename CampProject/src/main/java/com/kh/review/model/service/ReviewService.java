...
public class ReviewService {
	...
	public ReviewPageData reviewList(int reqPage) {
		...
		//이전 버튼
		if(pageNo != 1) {
			pageNavi += "<a class='btn btn-outline-dark' href='reviewList.do?reqPage="+(pageNo-1)+"'>이전</a>&emsp;";
		}
		...
		for(int i=0; i<pageNaviSize; i++) {
			...
			if(pageNo != reqPage) {
				...
				pageNavi += "<a class='btn btn-outline-dark' href='reviewList.do?reqPage="+pageNo+"'>"+pageNo+"</a>&emsp;";
			} else {
				pageNavi += "<span class='btn btn-outline-dark'>"+pageNo+"</span>&emsp;";
			}
